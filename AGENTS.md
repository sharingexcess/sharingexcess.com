...
- **Clean URLs:** Astro’s default `build.format: "directory"` serves pages at clean paths (e.g. `/about`, `/free-food/colorado`). Internal links are rewritten without `.html` when running `bun run generate`. Legacy `*.html` bookmarks are 301-redirected in production by [`public/serve.json`](public/serve.json) (`serve` host config — Astro config redirects require explicit routes and cannot wildcard-match dynamic free-food paths).
...
- **Free food pages** (`/free-food`, state/city/profile routes) fetch partner locations at build time from the Surplus public API (`GET /public/free_food/profiles`). Profile pages use `googlePlaceId` as the URL key: `/free-food/{state}/{city}/{placeId}`. Runtime client fetches may also use `GET /public/free_food/profiles/{placeId}` (single profile) and `GET /public/free_food/places/{placeId}/details` (Google proxy). Legacy API aliases at `/public/find_food/*` remain during transition. Production API: `https://api.sharingexcess.com`. Local dev (`bun run dev` / `import.meta.env.DEV`) uses `http://localhost:8080`. Override with `PUBLIC_API_SERVER_URL` or `SURPLUS_LOCAL_RUNTIME=1`.
